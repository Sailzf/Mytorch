...
# 每个Tensor对象在创建时，会通过set_creator方法记录生成它的操作(Function对象)以及相关的输入(inputs)。
# 当调用backward()时，方法首先初始化当前Tensor的梯度（通常从标量梯度1开始），
# 然后将所有相关的操作按生成顺序（generation）存储在一个堆中（heapq实现的优先队列）
# Tensor的creator首先被加入堆中，随后对每个操作的输入（inputs）进行检查，如果它们有自己的creator，就将这些creator加入堆中。
# 接着，程序通过循环从堆中依次弹出操作，调用操作的backward方法，利用链式法则计算输入的梯度，并累加到对应输入Tensor的梯度中（若梯度已存在，则累加）。
# 整个过程递归遍历计算图，直至所有相关操作完成。
                ...
                for x, gx in zip(f.inputs, gxs):
                    if x.requires_grad and gx is not None:
                        ...
                        if x.creator is not None:
                            add_func(x.creator) # 【对于当前操作的所有输入，如果它们有creator，将其加入堆中】
