...
class Conv2D(Module):
    """
    二维卷积层，支持前向传播，支持 numpy 和 cupy。
    """

    def __init__(self, in_channels: int, out_channels: int, kernel_size: Tuple[int, int],
                 stride: int = 1, padding: int = 0, device=None, dtype=None) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
        ...
        # 初始化权重和偏置
        kh, kw = kernel_size
        # self.weight = Parameter(Tensor.rand(out_channels, in_channels, kh, kw, requires_grad=True))
        scale = (2 / (in_channels * kh * kw)) ** 0.5  # Kaiming初始化
        self.weight = Parameter(Tensor.randn(out_channels, in_channels, kh, kw) * scale, **factory_kwargs)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # print(type(self.weight.data))  # 应输出 <class 'cupy._core.core.ndarray'>
        # print(type(col))  # 应输出 <class 'cupy._core.core.ndarray'>
        # print(type(col_W))  # 应输出 <class 'cupy._core.core.ndarray'>

        # 卷积运算
        out = xp.dot(col, col_W) + self.bias.data
        out = out.reshape(N, out_h, out_w, -1).transpose(0, 3, 1, 2)

        # 返回 Tensor 对象，支持计算图的自动构建
