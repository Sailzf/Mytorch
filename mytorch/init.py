...
def _calculate_fan_in_and_fan_out(tensor: Tensor) -> Tuple:
    ...
    if dimensions == 2:  # 线性层
        fan_in = tensor.size(1)  # 输入大小
        fan_out = tensor.size(0)  # 输出大小
    elif dimensions == 4:  # 卷积层
        # 对于卷积层，计算方式为：
        # fan_in = 输入通道数 × 卷积核高 × 卷积核宽
        # fan_out = 输出通道数 × 卷积核高 × 卷积核宽
        fan_in = tensor.size(1) * tensor.size(2) * tensor.size(3)
        fan_out = tensor.size(0) * tensor.size(2) * tensor.size(3)
    else:
        raise ValueError(f"Unsupported tensor dimensions: {dimensions}. Must be 2 (Linear) or 4 (Conv2d).")
