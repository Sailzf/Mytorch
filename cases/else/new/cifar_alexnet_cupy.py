...
class CIFAR10Dataset(Dataset):
    def __init__(self, cifar_dataset, cache_file=None):
        super().__init__()
    ...
    def _start_caching(self):
        ...
        # 使用tqdm创建进度条
        for img, target in tqdm(self.cifar_dataset, desc="Caching images", unit="img"):
...
# 初始化 SwanLab
run = swanlab.init(
    project="CIFAR10-AlexNet",
    experiment_name="CIFAR10-AlexNet-CuPy",
    config={
        "optimizer": "SGD",
        "learning_rate": 0.01,
        "batch_size": 128,
...
train_transform = transforms.Compose([
    transforms.Resize(224),  # AlexNet需要224x224的输入
...
train_cifar = datasets.CIFAR10('data', train=True, download=True, transform=train_transform)
test_cifar = datasets.CIFAR10('data', train=False, download=True, transform=val_transform)

train_dataset = CIFAR10Dataset(
    train_cifar,
    cache_file='data/cifar10/cache/train'
)

val_dataset = CIFAR10Dataset(
    test_cifar,
    cache_file='data/cifar10/cache/val'
)
...
model = AlexNet(num_classes=10, dropout_rate=run.config.dropout_rate)
